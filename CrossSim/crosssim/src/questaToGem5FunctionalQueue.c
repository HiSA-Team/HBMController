...
uint8_t qtg5_functional_init(struct QuestaToGem5FunctionalQueue * shq)
{
    if (!shq->is_init)
    {
        shq->head = 0;
...
uint8_t qtg5_functional_full(struct QuestaToGem5FunctionalQueue * shq)
{
    return shq->tail == shq->head && shq->id[shq->tail] != 0;
}

uint8_t qtg5_functional_empty(struct QuestaToGem5FunctionalQueue * shq)
{
    return shq->tail == shq->head && shq->id[shq->tail] == 0;
}

uint8_t qtg5_functional_enqueue(struct QuestaToGem5FunctionalQueue * shq, uint8_t id, uint8_t ack, uint64_t data, uint8_t clock_cycles)
{
    pthread_mutex_lock(&shq->queueMutex);
    // printf("questa_send(): starting...\n");

    // The queue is full, unlock the mutex and return
    if (qtg5_functional_full(shq))
    {
...
uint8_t qtg5_functional_dequeue(struct QuestaToGem5FunctionalQueue * shq, uint8_t* id, uint8_t* ack, uint64_t* data, uint8_t* clock_cycles)
{
