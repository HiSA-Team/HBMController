...
uint8_t g5tq_init(struct Gem5ToQuestaQueue * shq)
{
    if (!shq->is_init)
    {
        shq->head = 0;
...
uint8_t g5tq_full(struct Gem5ToQuestaQueue * shq)
{
    return shq->tail == shq->head && shq->id[shq->tail] != 0;
}

uint8_t g5tq_empty(struct Gem5ToQuestaQueue * shq)
{
    return shq->tail == shq->head && shq->id[shq->tail] == 0;
}

uint8_t g5tq_generate_id(struct Gem5ToQuestaQueue * shq)
{
    uint8_t found;
    uint8_t id;
...
uint8_t g5tq_enqueue(struct Gem5ToQuestaQueue * shq, uint64_t address, uint64_t data, uint8_t is_write)
{
    ...
    shq->id[shq->tail]       = g5tq_generate_id(shq);
...
uint8_t g5tq_dequeue(struct Gem5ToQuestaQueue * shq, uint8_t* id, uint64_t* address, uint64_t* data, uint8_t* is_write)
{
    pthread_mutex_lock(&shq->queueMutex);
    ...
    {
        pthread_mutex_unlock(&shq->queueMutex);
	    // printf("questa_receive(): empty queue, exiting...\n");
        return 0;
    }
    ...
    *is_write =   shq->is_write[shq->head];
    ...
    // printf("questa_receive(): id       = %" PRId8 "\n", *id);
    // printf("questa_receive(): address  = %" PRId64 "\n", *address);
    // printf("questa_receive(): data     = %" PRId64 "\n", *data);
    // printf("questa_receive(): is_write = %" PRId8 "\n", *is_write);
